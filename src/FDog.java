...
public class FDog extends Screen {
    ...
    public void gameUpdate(long deltaTime) {
        ...
        for (int i = 0; i < wallGroup.Size; i++) {
            if(doge.getRect().intersects(Walls.getWall(i).getRect())){
                doge.setPosX(Walls.getWall(i).getPosX()-Walls.getWall(i).getW());
                doge.setLive(false);
    ...
    public void gamePaint(Graphics2D g2) {
        ...
        Ground.Paint(g2);
        if (showHB) {
            Rectangle dogRect = doge.getRect();
            g2.setColor(Color.RED); // Set color for the dog's hitbox
            g2.drawRect((int) dogRect.getX(), (int) dogRect.getY(), (int) dogRect.getWidth(), (int) dogRect.getHeight());
            for (int i = 0; i < wallGroup.Size; i++) {
                Rectangle wallRect = Walls.getWall(i).getRect();
                g2.setColor(Color.BLUE); // Set color for the wall's hitbox
                g2.drawRect((int) wallRect.getX(), (int) wallRect.getY(), (int) wallRect.getWidth(), (int) wallRect.getHeight());
            }
        }
        ...
        g2.setColor(Color.decode("#511838"));
            g2.drawString("Highest score: "+Hscore, 20, 80);
    }
